...
class TransactionWindow(ctk.CTkFrame):
    ...
    def create_widgets(self):
        ...
        self.frm_table = ctk.CTkScrollableFrame(
            self,
            fg_color="#a6b985",
            bg_color="#648a64",
            width=860,
            scrollbar_button_color="#46685b",
            scrollbar_button_hover_color="#213435",
        )
        ...
        def options_column_filter(choice):
            ...
            if choice == "Index":
                ...
                self.entry_filter.configure(
                    placeholder_text="Enter the transaction description"
                )
            elif choice == "Amount":
                self.refresh(dataframe)
                self.option_filter.configure(state="normal")
        ...
        self.column_filter = ctk.CTkComboBox(
            self.frm_table,
            font=ctk.CTkFont(size=13, weight="bold"),
            button_color="#648a64",
            button_hover_color="#213435",
            width=100,
            values=column_filter_values,
            command=options_column_filter,
        )
        ...
        def options_filter(choice):
            ...
            if choice == "==":
                ...
                self.refresh(dataframe)
                self.entry_filter.configure(
                    placeholder_text="Values higher than the entered number will be filtered out"
                )
                if self.entry_filter.get():
                    self.entry_filter.delete(0, "end")
                self.option_filter.focus_set()
                ...
                self.refresh(dataframe)
                self.entry_filter.configure(
                    placeholder_text="Values lower than the entered number will be filtered out"
                )
                if self.entry_filter.get():
                    self.entry_filter.delete(0, "end")
                self.option_filter.focus_set()
                ...
                self.refresh(dataframe)
                self.entry_filter.configure(
                    placeholder_text="Values greater than or equal to the entered number will be filtered out"
                )
                if self.entry_filter.get():
                    self.entry_filter.delete(0, "end")
                self.option_filter.focus_set()
                ...
                self.refresh(dataframe)
                self.entry_filter.configure(
                    placeholder_text="Values less than or equal to the entered number will be filtered out"
                )
                if self.entry_filter.get():
                    self.entry_filter.delete(0, "end")
                self.option_filter.focus_set()
            ...
            if choice == "Today":
                ...
                self.entry_filter.insert(
                    0, f"{seven_days_ago.isoformat()} - {self.today.isoformat()}"
                )
                self.entry_filter.configure(state="disabled")
                ...
                self.entry_filter.insert(
                    0, f"{first_day_month.isoformat()} - {self.today.isoformat()}"
                )
                self.entry_filter.configure(state="disabled")
                df_filtered = self.data[
                    self.data["Date"] >= first_day_month.isoformat()
                ]
                table_data = [tuple(value) for value in df_filtered.values.tolist()]
                self.refresh(table_data)
                ...
                if self.entry_filter.get():
                    self.entry_filter.delete(0, "end")
                first_day_month = self.today.replace(day=1)
                first_day_previous_month = first_day_month.replace(
                    month=self.today.month - 1
                )
                last_day_previous_month = first_day_month - timedelta(days=1)
                self.entry_filter.insert(
                    0,
                    f"{first_day_previous_month.isoformat()} - {last_day_previous_month.isoformat()}",
                )
                self.entry_filter.configure(state="disabled")
                df_filtered = self.data[
                    (self.data["Date"] >= first_day_previous_month.isoformat())
                    & (self.data["Date"] <= last_day_previous_month.isoformat())
                ]
                table_data = [tuple(value) for value in df_filtered.values.tolist()]
                self.refresh(table_data)
                ...
                self.entry_filter.insert(
                    0, f"{first_day_year.isoformat()} - {self.today.isoformat()}"
                )
                self.entry_filter.configure(state="disabled")
                ...
                if self.entry_filter.get():
                    self.entry_filter.delete(0, "end")
                self.entry_filter.configure(
                    placeholder_text="Example: 2026-03-01 to 2026-03-07 (Dates separated by 'to')"
                )
                self.option_filter.focus_set()
        ...
        # SECTION: SURVEY
        # ------------------------------
        frm_edit_create_delete_transactions = ctk.CTkFrame(
            self, fg_color="#a6b985", bg_color="#648a64", width=300, height=180
        )
        frm_edit_create_delete_transactions.grid(row=1, column=0, sticky="nsew", padx=5)

        self.lbl_title = ctk.CTkLabel(
            frm_edit_create_delete_transactions,
            text="Create",
            fg_color="#46685b",
            text_color="#e1e3ac",
            corner_radius=20,
        ...
        self.txtbox_concept = ctk.CTkTextbox(
            frm_edit_create_delete_transactions,
            font=ctk.CTkFont(size=13, weight="bold"),
            width=200,
            height=80,
        ...
        self.entry_amount = ctk.CTkEntry(
            frm_edit_create_delete_transactions,
            font=ctk.CTkFont(size=13, weight="bold"),
            placeholder_text="Example: 10.00",
            width=150,
        )
        ...
        def option_cbbox_category(choice):
            self.entry_amount.configure(
                border_width=2, border_color=self.colors_cells[choice][0]
            )
            self.cbbox_category.configure(
                border_width=2, border_color=self.colors_cells[choice][0]
            )
            self.amount_control()
        ...
        self.cbbox_category = ctk.CTkComboBox(
            frm_edit_create_delete_transactions,
            font=ctk.CTkFont(size=13, weight="bold"),
            button_color="#648a64",
            button_hover_color="#213435",
        ...
        border = (
            self.colors_cells[self.cbbox_category.get()][0]
            if not self.empty_table
            else "gray"
        )
        ...
        self.lbl_curent_balance = ctk.CTkLabel(
            frm_edit_create_delete_transactions,
        ...
        self.btn_category = ctk.CTkButton(
            ...
            hover_color="#213435",
            border_width=2,
            command=self.create_edit_delete_category,
        )
    ...
    def execute_filter(self, event):
        ...
        if self.data["Amount"].dtype == "float64":
            dataframe = [tuple(value) for value in self.data.values.tolist()]
            dataframe = [
                (value[0], value[1], value[2], f"{value[3]:.2f}", value[4])
        ...
        if option_filter_value == "==" and column_filter_value == "Index":
            if "," not in actual_value and actual_value.strip().isdigit():
                ...
                indexes = [
                    int(index.strip()) - 1
                    for index in actual_value.split(",")
        ...
        if option_filter_value == "Custom range":
            if "to" in actual_value and actual_value.count("to") == 1:
                ...
                if len(dates) == 2:
                    df_filtered = self.data[
                        (self.data["Date"] >= dates[0])
                        & (self.data["Date"] <= dates[1])
                    ]
                    table_data = [tuple(value) for value in df_filtered.values.tolist()]
                    self.refresh(table_data)
            elif actual_value == "":
        ...
        if column_filter_value == "Concept":
            if actual_value:
                df_filtered = self.data[
                    self.data["Concept"].str.contains(actual_value, case=False)
                ]
                table_data = [tuple(value) for value in df_filtered.values.tolist()]
                self.refresh(table_data)
            else:
            ...
            if "," not in actual_value and actual_value.strip().isdigit():
                ...
                try:
                    ...
                    table_data = [
                        ...
                        for value in df_filtered.values.tolist()
                    ]
                    self.refresh(table_data)
                except Exception as e:
                    self.entry_filter.configure(border_width=2, border_color="red")
            ...
            if actual_value:
                try:
                    ...
                    if self.data["Amount"].dtype != "float64":
                        self.data["Amount"] = [
                            float(i.replace("$", ""))
                            for i in self.data["Amount"].values.tolist()
                        ]
        ...
        if option_filter_value == "Name":
            if actual_value:
                df_filtered = self.data[
                    self.data["Category_ID"].str.contains(actual_value, case=False)
                ]
                table_data = [tuple(value) for value in df_filtered.values.tolist()]
                self.refresh(table_data)
            else:
        ...
        if actual_mode in ("Edit", "Delete"):
            if amount != None:
                if color_categorie == "#BC6C25":
                    ...
                    amount = abs(amount)
                self.amount_column_data[self.table_transactions.row_clicked - 1] = (
                    amount
                )
    ...
    def refresh(self, table_data):
        if self.table_transactions:
            ...
            self.table_transactions = TableTransactions(
                self.frm_table, table_data, self
            )
            self.table_transactions.grid(row=1, column=0, columnspan=3)
    ...
    def create_edit_delete_category(self):
        ...
        if actual_state == "Create" or actual_state == "Edit":
            if new_date == "":
                ...
                self.after(
                    2000, lambda: self.calendar.date_entry.configure(border_width=0)
                )
                ...
                self.after(2000, lambda: self.btn_category.configure(state="normal"))
                return
            elif new_category not in self.parent_ref.frm_category.categories_names:
        ...
        if actual_state == "Create":
            ...
            self.entry_filter.configure(state="normal")
            self.option_filter.configure(state="normal")
            self.dbt.insert_values(
                Transaction(new_date, new_concept, new_amount, new_category)
            )
            self.load_data()
            self.refresh([tuple(value) for value in self.data.values.tolist()])
            self.parent_ref.frm_category.information_labels()
            self.load_table_data()
            self.parent_ref.frm_category.load_data()
            self.calendar.date_entry.delete(0, "end")
            self.txtbox_concept.delete("1.0", "end")
            self.entry_amount.delete(0, "end")
            self.calendar.date_entry.focus()
            self.cbbox_category.set(self.parent_ref.frm_category.categories_names[0])
    ...
    def load_data(self):
        if not self.empty_table:
            ...
            self.df = pd.DataFrame(
                self.transactions,
                columns=["Index", "Date", "Concept", "Amount", "Category_ID"],
            )
    ...
    def load_table_data(self):
        ...
        if self.current_balance >= 0:
            self.lbl_curent_balance.configure(
                text=f"Current Balance: {self.current_balance:.2f}$", fg_color="#648a64"
            )
        else:
            self.lbl_curent_balance.configure(
                text=f"Current Balance: {self.current_balance:.2f}$", fg_color="orange"
            )
